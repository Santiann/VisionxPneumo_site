...
const FAQ = ({ perguntasFrequentes, visibleQuestion, toggleVisibility }) => (
  <div className='bg-white w-auto p-6 rounded-3xl m-6'>
    ...
    {perguntasFrequentes.map((item, index) => (
      <div key={index}>
        <h3 onClick={() => toggleVisibility(index)} className='cursor-pointer font-semibold'>
          {item.title}
        </h3>
        {visibleQuestion === index && <p>{item.description}</p>}
      </div>
    ))}
  </div>
);
...
const EmailForm = ({ email, setEmail, handleEmailSubmit }) => (
  <div id="formEmailEl" className='bg-white lg:w-8/12 mb-16 rounded-2xl p-6 m-6'>
    ...
    <form onSubmit={handleEmailSubmit} className='flex flex-col gap-2 h-auto w-full'>
      ...
      <button type='Submit' className='w-28 text-center cursor-pointer mt-4 bg-gray-800 text-white font-medium py-2 px-4 rounded-full shadow-md hover:bg-gray-700 hover:shadow-lg transition duration-300'>Enviar</button>
    </form>
  </div>
);
...
const WhatsAppForm = ({ handleWhatsAppSubmit, phone, setPhone }) => (
  <div id="formPhoneEl" className='bg-white lg:w-4/12 h-auto mb-16 rounded-3xl p-6 m-6'>
    <h2 className='font-semibold text-xl text-gray-800'>Pelo WhatsApp</h2>
...
const Suporte = ({ auth }) => {
  ...
  const [phone, setPhone] = useState("5545985053873");
